/*
   Сервачек написан на основании примера из arduino examples FSBrowser
   подправлен Третьяковы Сергеем
   ссылка на видео howto https://www.youtube.com/watch?v=J1QkMad29zU
   ссылка на сайт с описанием howto http://esp8266-arduinoide.ru/fswebserver/
    ...
    плагин для загрузки SPIFFS https://github.com/esp8266/arduino-esp8266fs-plugin/releases/tag/0.3.0 
    архив распоковывается в папку arduino\tools
    и дает доступ к SPIFFS.
    ==
    Располагаем в папке с этим скетчем папку data ( с html проектом)
    и загружаем через Инструменты - esp8266 Sketch Data Upload
    архив с исходниками этого скетча и папки data расположен уже у меня на git:
    https://github.com/nicelight/esp_SPIFF_Web_server

    --- Внутри проекта имем:---
    поднятие точки доступа для ввода данных коннекта к wifi домашнему
    файловый сервер на котором расположены web страницы,
    их можно изменять\ подгружать через браузер
    страницы расположены в памяти SPIFS
    ОГОНЬ !!!

    -- todo list: --
    сделать OTA загрузку. хотя не принципиально, ибо прошивку можно лить через web интерфейс
     так и не смог реализовать обработку нажатия кнопок на сервер так что 
     дпнный проект удобен если у нас много красивых страниц 
     а для работы с железом как то пока не складывается у меня.. надо разобраться
*/
